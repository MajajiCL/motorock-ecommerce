...
export default function Navbar({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#e4e4e7]">
      {/* Barra de Notificación Superior MotoRock Red */}
      <div className="bg-[#121214] text-white text-[11px] py-1.5 px-4 font-normal">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2">
          <div className="flex items-center gap-2">
            <span className="bg-[#e60000] text-white text-[9px] font-bold px-2 py-0.5 rounded-full">
              MOTOROCK APP
            </span>
          ...
          <div className="flex items-center gap-4 text-zinc-300">
            ...
            <button
              ...
              className="text-[#e60000] hover:underline font-semibold cursor-pointer"
            >
              Descargar App Móvil
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 max-w-lg relative hidden md:block" ref={searchRef}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[#f8f9fa] border border-[#e4e4e7] focus:border-[#e60000] text-xs text-[#121214] pl-9 pr-20 py-2.5 rounded-full focus:outline-none focus:ring-1 focus:ring-[#e60000] transition-all placeholder:text-zinc-400 font-medium"
            />
            {searchQuery ? (
              <button
                onClick={() => {
              ...
              <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[10px] font-semibold text-[#e60000] bg-red-50 px-2 py-0.5 rounded-full">
                726 repuestos
              </span>
            )}
          </div>
          ...
          {showSearch && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#e4e4e7] rounded-2xl shadow-motorock overflow-hidden z-50 divide-y divide-zinc-100">
              ...
              {suggestions.map((item) => (
                <div
                  ...
                  className="p-3 hover:bg-red-50/50 flex items-center gap-3 cursor-pointer transition-colors"
                >
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-semibold text-[#121214] truncate">{item.name}</h4>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            onClick={onOpenAppModal}
            className="hidden sm:flex items-center gap-2 bg-[#e60000] hover:bg-[#cc0000] text-white px-4 py-2 rounded-full font-semibold text-xs shadow-motorock-red transition-all cursor-pointer"
          >
            <Smartphone size={14} />
          ...
          <button
            onClick={onOpenCart}
            className="flex items-center gap-2 bg-white border border-[#e4e4e7] hover:border-[#e60000] text-[#121214] px-3.5 py-2 rounded-full font-bold text-xs shadow-sm transition-all cursor-pointer"
          >
            <ShoppingCart size={15} className="text-[#e60000]" />
            <span className="hidden sm:inline">Carrito</span>
            {cartCount > 0 && (
              <span className="bg-[#e60000] text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-sm">
                {cartCount}
              </span>
            )}
          </button>
