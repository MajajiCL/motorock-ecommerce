        ...
        <div className="overflow-y-auto flex-1 p-6 md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <div className="aspect-square bg-[#f6f6fa]/70 rounded-[24px] border border-[#e5e5eb] p-6 flex items-center justify-center relative overflow-hidden">
                ...
                />
                {product.onSale && (
                  <span className="absolute top-4 left-4 bg-[#151581] text-white font-semibold text-[10px] px-3 py-1 rounded-full shadow-sm">
                    OFERTA
                  </span>
                )}
              </div>
                  ...
                  {images.map((img, idx) => (
                    <button
                      ...
                      className={`w-14 h-14 rounded-xl border p-1 bg-white transition-all flex-shrink-0 cursor-pointer ${
                        selectedImg === idx ? "border-[#151581] ring-2 ring-[#151581]/20" : "border-[#e5e5eb] hover:border-slate-400"
                      }`}
                    >
            ...
            <div className="flex flex-col justify-between space-y-5">
              <div>
                <div className="flex items-center gap-2 text-xs mb-1.5">
                  <span className="bg-[#f6f6fa] text-[#151581] font-semibold px-3 py-0.5 rounded-full text-[10px] uppercase border border-[#e5e5eb]">
                    {product.categories?.[0]?.name || "Repuestos"}
                  </span>
                  <span className="text-[#a1a1cd] font-mono text-[11px]">SKU: {product.sku}</span>
                </div>

                <h2 className="text-xl sm:text-2xl font-semibold text-[#151581] leading-tight">
                  {product.name}
                </h2>
                ...
                <div className="mt-4 p-4 rounded-[20px] bg-[#f6f6fa] border border-[#e5e5eb] space-y-1">
                  <div className="flex items-baseline gap-3">
                    ...
                    {product.onSale && (
                      <span className="text-sm text-[#a1a1cd] line-through">
                ...
                <div className="mt-3">
                  {product.inStock ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 bg-slate-100 px-3.5 py-1 rounded-full">
                          ...
                          <button
                            ...
                            className={`px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all border cursor-pointer ${
                              isSelected
                                ...
                                : "bg-white border-[#e5e5eb] text-[#292824] hover:border-slate-400"
                            }`}
                          >
                            {attrText}
                          </button>
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  <div className="flex items-center bg-[#f6f6fa] border border-[#e5e5eb] rounded-full p-0.5">
                    <button
                      ...
                      className="w-7 h-7 bg-white text-slate-800 rounded-full font-bold hover:bg-slate-200 flex items-center justify-center text-xs"
                    >
                      -
                    </button>
                    <span className="w-8 text-center text-xs font-bold text-[#151581]">{quantity}</span>
                    <button
              ...
              <div className="space-y-2.5 pt-3 border-t border-slate-100">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  <button
                    ...
                    disabled={!product.inStock}
                    className="flex items-center justify-center gap-2 bg-[#151581] hover:bg-[#0e0e5a] text-white font-semibold py-3 px-4 rounded-full text-xs shadow-sm transition-all cursor-pointer"
                  >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2 bg-emerald-50 hover:bg-emerald-100 text-[#00bb76] border border-emerald-100 py-2.5 px-4 rounded-full font-semibold text-xs transition-colors w-full"
                >
          ...
          {product.description && (
            <div className="mt-6 pt-6 border-t border-slate-100">
              ...
              <div className="text-slate-600 text-xs leading-relaxed bg-[#f6f6fa] p-4 rounded-2xl border border-[#e5e5eb] whitespace-pre-line">
                {product.description}
              </div>
            </div>
          )}
